...
from encryption_helper import read_encrypted
from history import *
from etherdelta_api import *
from asset import *
...
class Client(object):
    ...
    def __init__(self, encry_pass, file_name):

        results = read_encrypted(encry_pass,file_name)
        print(results)
        self.API_KEY = "4TupCIrghQoMy2PpEUESQY4LmFfXIxHJlRJSD9cIW71CitQRkXEKSTZndrz4SZ3n"
        self.API_SECRET = results
    ...
    def binance_trades_with(self, symbol):
        binance_symbol = symbol+ "ETH"
        try:
            my_trades = self.get_my_trades(symbol=binance_symbol)
        except:
            return
        for trade in my_trades:
            amount = float(trade['qty'])
            eth_amount = float(trade['qty']) * float(trade['price'])
            is_buyer = trade["isBuyer"]
            if is_buyer is True:
                amount -= float(trade["commission"])
                print("--------BUY  {} : {} ETH   --->  amount: {} cost {} {}".format(symbol,str(eth_amount),str(amount),str(trade["commission"]),symbol))
                addBuyHistory(symbol,"ETH",eth_amount,amount)
            else:
                eth_amount -= float(trade["commission"])
                print("--------SELL {} : {} ETH   --->  amount: {} cost {} ETH".format(symbol,str(eth_amount),str(amount),trade["commission"]))
                addSellHistory(symbol,'ETH',eth_amount,amount)

    def binance_trades(self):
        print("-------------------Binance Trades History----------------")
        for token in ERC20_tokens:
            self.binance_trades_with(token)

    def binance_bal(self):
        print("----------Binance Balance----------")
        balances = self.get_account()['balances']
        for balance in balances:
            total_bal = float(balance['free']) + float(balance['locked'])
            if (total_bal >= 0.1):
                print(balance['asset'] + ":   " + str(total_bal))
                addAsset(balance['asset'],total_bal)
